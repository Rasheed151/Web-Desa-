...
<div class="content-tiga" id="content2">
    <table class="table-tiga">
        ...
        <tbody>
            @if(isset($kakSedia) && count($kakSedia) > 0)
                @foreach ($kakSedia as $data)
                    <tr>
                        <td>{{ $data->noKeg }}</td>
            ...
            @else
                <tr>
                    <td colspan="11" class="no-data">Tidak ada data KAK</td>
                </tr>
            @endif
        </tbody>
    </table>
    <a href="{{ route('kakSedia.create') }}" class="btn-primary">Tambahkan Data</a>
</div>
@endsection
@section('script')
