...
<div class="content-tiga" id="content2">
    <table class="table-tiga">
        ...
        <tbody>
            @if(isset($kakSwa) && count($kakSwa) > 0)
                @foreach ($kakSwa as $data)
                    <tr>
                        <td>{{ $data->no }}</td>
                        <td>{{ $data->tgglMulai }}</td>
                        <td>{{ $data->caraLaksana }}</td>
                        <td>{{ $data->ketuaTpk }}</td>
                        <td>{{ $data->namaKasi }}</td>
                        <td>
                            <a href="{{ route('kakSwa.show', $data->id) }}" class="btn">
                                <img src="/img/info.png" alt="Info" style="width:24px; height:24px; border: none; background: none;">
                            </a>

                            <!-- Edit Button -->
                            <a href="{{ route('kakSwa.edit', $data->id) }}" class="btn">
                            <img src="/img/edit.png" alt="Edit" style="width:24px; height:24px; border: none; background: none;">
                            </a>

                            <!-- Delete Button -->
                            <form action="{{ route('kakSwa.destroy', $data->id) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
    ...
    <a href="{{ route('kakSwa.create') }}" class="btn-primary">Tambahkan Data</a>
</div>
@endsection
@section('script')
