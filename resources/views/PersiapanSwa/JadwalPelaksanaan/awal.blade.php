...
<div class="satu">
    <div class="title">
        <h1>Jadwal Pelaksanaan Secara Swakelola</h1>
    </div>
</div>
...
<div class="content-tiga" id="content2">
            ...
            @if(isset($jadwal) && count($jadwal) > 0)
                @foreach ($jadwal as $data)
                    <tr>
                        ...
                        <td>{{ $data->jdwlLaksana }}</td>
                        <td>{{ $data->biaya }}</td>
                        <td>{{ $data->namaKasi }}</td>
                        <td>
                            <a href="{{ route('jadwalSwa.show', $data->id) }}" class="btn">
                                <img src="/img/info.png" alt="Info" style="width:24px; height:24px; border: none; background: none;">
                            </a>

                            <!-- Edit Button -->
                            <a href="{{ route('jadwalSwa.edit', $data->id) }}" class="btn">
                            <img src="/img/edit.png" alt="Edit" style="width:24px; height:24px; border: none; background: none;">
                            </a>

                            <!-- Delete Button -->
                            <form action="{{ route('jadwalSwa.destroy', $data->id) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
    ...
    <a href="{{ route('jadwalSwa.create') }}" class="btn-primary">Tambahkan Data</a>
</div>
@endsection
@section('script')
